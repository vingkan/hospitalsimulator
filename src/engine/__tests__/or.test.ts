...
describe('OR module', () => {
  ...
  it('getFitnessCriteria returns utilization, cancellation, and margin ranges', () => {
    ...
    expect(utilization!.max).toBeLessThanOrEqual(0.90)
    ...
    const margin = criteria.find(c => c.metric === 'surgicalMargin')
    expect(margin).toBeDefined()
    expect(margin!.min).toBeGreaterThanOrEqual(0.10)
    expect(margin!.max).toBeLessThanOrEqual(0.30)
  })
})
